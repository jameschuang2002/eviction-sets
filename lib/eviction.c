...
// Setting this to true tells access_loop to stop (when running in a separate thread)
...
// Calculates the cache hit threshold by timing a memory access both with and without evicting it.
// If cl_set isn't an eviction set for victim, this won't work.
...
// Calculates the cache hit threshold by timing a memory access both with and without flushing it.
...
{
    cl_set->size++;
    cl_set->cache_lines = reallocarray(cl_set->cache_lines, cl_set->size, sizeof(CacheLine));
...
void shuffle_lines(CacheLineSet *cl_set)
{
    for (int i = 0; i < cl_set->size - 1; i++)
    {
        ...
        memcpy(&(cl_set->cache_lines[i]), &(cl_set->cache_lines[j]), sizeof(CacheLine *));
...
CacheLineSet *inflate(uint8_t *victim, int max_size, int samples, uint64_t threshold)
{
...
uint64_t evict_and_time(CacheLineSet *cl_set, uint8_t *victim, NumList *timings, bool use_siblings)
{
    ...
    for (int i = 0; i < cl_set->size; i++)
    {
        ...
        // It includes all old cache lines, as well as the immediately preceding/subsequent (sibling) lines
        if (use_siblings)
        {
...
bool reduce2(CacheLineSet *cl_set, CacheLineSet *reserve, uint8_t *victim, int samples, uint64_t threshold, int bins)
{
    ...
    // Continue until we fail to reduce 5 times in a row, while the working set is an eviction set
    while (strikes < 5)
    {
        // Split the eviction set into many bins of equal (except for potentially the last one) size.
        // Leave each of these bins out, considering the remainder as an eviction set and measuring
        // its performance.
        ...
        for (int i = 0; i < starts->length; i++)
        {
            ...
            for (int j = 0; j < cl_set->size; j++)
            {
                ...
                // Skip cache lines which were previously left out without degrading performance
                bool found = false;
        ...
        // If some ranges could be left out, remove all cache lines in those ranges, saving them in reserve
        else
        {
            strikes = 0;
...
// Returns a minimal eviction set for the victim, using a pre-computed eviction threshold
bool get_minimal_set(uint8_t *victim, CacheLineSet **cl_set, uint64_t threshold)
{
...
int evict_time_multi(CacheLineSet *cl_set, uint8_t *victim, uint64_t threshold, bool use_siblings)
{
    int count = 0;
    for (int i = 0; i < SAMPLES; i++)
    {
...
int same_cache_set(uint8_t *cl1, uint8_t *cl2, CacheLineSet *cl_set, uint64_t threshold)
{
...
CacheLine *allocate_matching(uint8_t *victim, int matching_bits)
{
    ...
    while (!match_cache_set((uint8_t *)aligned_page, (uint8_t *)victim, matching_bits))
    {
...
CacheLineSet **generate_sets(int num_sets, uint8_t *victim_page_offset)
{
    ...
    uint8_t dummy;
    CacheLineSet *initial_set = inflate(&dummy, INITIAL_SIZE, SAMPLES, INITIAL_THRESHOLD);
    ...
    push_cache_line(unique_lines, allocate_matching(victim_page_offset, MATCHING_BITS));
    ...
    get_minimal_set((uint8_t *)(unique_lines->cache_lines[0]), &probe_sets[0], threshold);

    // How well each eviction set evicts the victim
    NumList *eviction_rates = new_num_list(num_sets);
    push_num(eviction_rates, evict_time_multi(probe_sets[0], victim_page_offset, threshold, false));
    ...
    // Continue generating unique cache lines until we have one for every possible cache set
    while (unique_lines->size < num_sets)
    {
        ...
        {
            // If the new cache line was in the same cache set as an existing one, move on
            int match = 2;

            while (true)
            {
                // printf("Testing set %u on new line...\n", i);
                match = same_cache_set((uint8_t *)(unique_lines->cache_lines[i]), (uint8_t *)cl_new, probe_sets[i], threshold);
                if (match != 2 && match != 3)
                {
                    break;
                }
                // If the saved eviction set no longer works, generate another one to stop it from getting stuck
        ...
        // If the new cache line was unique, add it to the list, along with its minimal eviction set
        if (unique)
        {
            ...
            push_num(eviction_rates, evict_time_multi(*new_cl_set, victim_page_offset, threshold, false));
            printf("Victim eviction rates for each generated set:\n");
            for (int i = 0; i < eviction_rates->length; i++)
            {
                printf("Victim eviction rate %u: %lu/%u for set from cache line ", i, eviction_rates->nums[i], SAMPLES);
            ...
            for (int i = 0; i < unique_lines->size; i++) 
            {
                ...
                for (int j = 0; j < pas[i]->length; j++)
                {
                    ...
                    if (old_pa != new_pa)
                    {
                        ...
                        printf("Error! Physical address changed in set %u from original value %lx to %lx.\n", i, old_pa, new_pa);
